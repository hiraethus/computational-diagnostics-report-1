...
In the case of the score matrices, we see that $\Sexpr{score.mismatches}$ mismatches were found between the alignment 
score matrices where a penalty was made for a mismatch.
